        ...
        <nav className="font-inter mx-auto h-auto lg:relative lg:top-0">
          <div className="flex flex-col px-6 py-6 lg:flex-row lg:items-center lg:justify-between lg:px-10 lg:py-4 xl:px-20">
            <Link to="#" onClick={closeMenu}>
              <img src={Logo} alt="" className="inline-block h-full w-full" />
            </Link>

            {/* Burger Men u */}
            <Link
              to="#"
              className="absolute right-8 top-14 lg:hidden animate-bounce"
              onClick={toggleMenu}
            >
              <svg
                width="1.25rem"
            ...
            {isMenuOpen && (
              <div className="mt-10 flex flex-col items-start space-y-8 lg:mt-0 lg:flex lg:flex-row lg:items-center lg:space-x-3 lg:space-y-0">
                ...
                <Link
                  to="/PropertyListing"
                  onClick={closeMenu}
                  className="ml-5 rounded-full bg-orange-600 px-6 py-4 text-center font-semibold text-white transition hover:bg-[#2d6ae0] md:ml-10 lg:ml-4"
                >
                  Property Listings
                </Link>
              </div>
